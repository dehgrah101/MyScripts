...
foreach ( $Subscription in $(Get-AzSubscription | Where-Object { $_.State -ne "Disabled" }) ) {
    ...
    $nets = Get-AzVirtualNetwork -Name $Vnet -ResourceGroupName $ResourceGroupName
    foreach ($net in $nets) {
        $snets = $net.Subnets
...
$outputfinal | Export-Csv C:\users\Public\NETS_"$((Get-Date).ToString("yyyyMMdd_HHmmss")).csv" -NoTypeInformation